...
// Include Logger and the implementation ONCE in main.cpp
#include "Logger.h"
...
void loop() {
    ...
    if (currentTime - lastStatusTime > 30000) {
        ...
        // Logger statistics
        auto metrics = Logger::getInstance().getMetrics();
        Logger::getInstance().log(ESP_LOG_INFO, "Main", 
                                  "Logger stats - Total: %lu, Dropped: %lu, Failed: %lu",
                                  metrics.totalLogs, metrics.droppedLogs, 
                                  Logger::getInstance().getFailedMutexAcquisitions());
