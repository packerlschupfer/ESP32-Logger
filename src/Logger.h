...
private:
    ...
    static constexpr size_t TAG_POOL_SIZE = 64;  // Pool for tag strings
    struct TagEntry {
        const char* tag;  // Points to string in tagStringPool
    ...
    // Tag-level filtering with minimal memory overhead
    std::unordered_map<std::string_view, esp_log_level_t> tagLevels;
    mutable SemaphoreHandle_t tagMutex;
    char tagStringPool[TAG_POOL_SIZE * 16];  // Storage for tag strings
    size_t tagPoolOffset = 0;
