...
> **CRITICAL FIX**: NonBlockingConsoleBackend now prevents 10+ second system freezes! See How to Use Non-Blocking Logger
...
**Important**: This Logger implementation uses heap allocation instead of Thread-Local Storage (TLS) to avoid the "17KB per task" memory waste problem. See SINGLETON_MEMORY_WASTE_PROBLEM.md for details.
...
See PROFESSIONAL_LOGGER.md for detailed professional features documentation.
...
```bash
cd examples/with_custom_logger
...
See How to Use Non-Blocking Logger for detailed configuration and best practices.
...
Using Logger across multiple libraries? See Multi-Library Usage Guide for best practices and examples.
...
See the detailed migration guide for step-by-step instructions and best practices.
...
This occurs when your application defines its own LOG_* macros that conflict with LogInterface.h. See docs/APPLICATION_LOG_MACRO_CONFLICTS.md for the solution.
